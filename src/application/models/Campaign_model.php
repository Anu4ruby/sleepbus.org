...
class Campaign_model extends CI_Model {
  ...
  public function GetAllCampaigns() {
   ...
   $sql = "select * from " . USER_CAMPAIGNS . " ORDER BY campaign_name";
  ...
  public function GetListOfCampaigns() {

   $sql = "select campaign_name, campaign_goal, url, CHAR_LENGTH(campaign_name) as title_length, IFNULL(image_file, 'BannerBus1.jpg') as image_file, IFNULL(FORMAT(sum(donations.paid_amount),2), 0) as raised_amount from user_campaigns left join donations on donations.campaign_id = user_campaigns.id   left join campaign_images on campaign_images.campaign_id = user_campaigns.id group by campaign_name
";
  ...
  public function GetTotalRaised($campaign_id) {
    $sql = "select sum(d.paid_amount) as amount from ".USER_CAMPAIGNS." uc inner join ".DONATIONS." d on uc.id=d.campaign_id where d.donation_type='campaign' and uc.id = '".$campaign_id."'";
