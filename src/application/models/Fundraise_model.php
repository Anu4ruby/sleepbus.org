...
class Fundraise_model extends CI_Model {
  ...
  public function GetFundraiseFormAttributes($values,$unit_fund,$campaign_type='') {
    ...
    $attribute['campaign_name'] = array(
      'name' => 'campaign_name',
      'id' => 'campaign_name',
      'value' => $values['campaign_name'],
      'class'=>'form-control',
      'placeholder'=>'Give your campaign a name',
      "onKeyUp"=>"return CountCharacters('campaign_name','limit2','80')");

    $attribute['limit2'] = array(
      'name'=> 'limit2',
      'id'=> 'limit2',
      'value' => strlen($values['campaign_name']),
      'readonly'=>'readonly',
      'size'=>'2',
      'tabindex'=>'-1');

    $attribute['campaign_goal'] = array(
      'name'=>'campaign_goal', 
      'id'=> 'campaign_goal', 
      'value'=>$values['campaign_goal'],
      'placeholder'=>number_format(($unit_fund)*10,2),
      'class'=>'form-control');


    $attribute['month'] = array(
      'name'=>'month', 
      'id'=> 'month', 
      'value'=>$values['month'],
      'placeholder'=>'MM',
    ...
    if(!empty($campaign_type)) {
      ...
      if(count($attribute['campaign_images']) > 0) {
        foreach($attribute['campaign_images'] as $image) {
          ...
          $attribute['campaign_image_'.$image['id']] = array(
            'name' => 'campaign_image', 
            'id' => 'campaign_image_'.$image['id'], 
            'value' => $image['id'],'checked'=>$checked);
        }
      }
  ...
  public function GetCampaignTypes() {
    ...
    return $this->db_query->FetchInformation(CAMPAIGN_TYPE,"","status='1' and id!='1'  order by position"); 
  }
